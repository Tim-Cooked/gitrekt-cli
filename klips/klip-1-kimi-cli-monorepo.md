...
# KLIP-1: Move Kosong and PyKAOS to Gitrekt CLI Monorepo

下面是一份「可执行的操作计划」，把我们前面确定的方案全部串起来，并加入你新补充的 tag 规则（`kosong-0.20.0`；`pykaos-0.2.0`；`Gitrekt-cli` 仍是 `0.68`/`0.68.1` 这种纯数字）。

## 1. 确定目标目录与命名

先定死，后面所有脚本/CI 都依赖它。

1. monorepo（目标仓库）仍然叫 `Gitrekt-cli`，且 `Gitrekt-cli` 包仍放在仓库根目录（保持你现在的结构/习惯）。
2. `kosong` 放到 `packages/kosong`，`pykaos` 放到 `packages/kaos`（目录叫 `kaos`，但 Python 包/发行名是 `pykaos`）。
3. 三个包的 `project.name`（PyPI 包名）分别是：`Gitrekt-cli`、`kosong`、`pykaos`。
4. tag 约定：
    - `Gitrekt-cli`：`0.68` / `0.68.1`（纯数字开头）
...
1. 在仓库根 `pyproject.toml`（`Gitrekt-cli`）里开启 workspace：`tool.uv.workspace.members = ["packages/kosong", "packages/kaos"]`。
2. 在仓库根 `pyproject.toml`（`Gitrekt-cli`）里加 `tool.uv.sources`，把依赖映射到 workspace member：
    - `kosong = { workspace = true }`
    - `pykaos = { workspace = true }`
3. `Gitrekt-cli` 的对外依赖（`project.dependencies`）写「发布后要生效的版本范围」，例如依赖 `kosong`、`pykaos` 的范围约束（你们自己决定兼容策略，上界不要省略）。开发时 uv 会用 workspace 里的本地包覆盖同名依赖；发布时用户仍会从 PyPI 拉对应版本。

## 3. 把 kosong、kaos 迁入 monorepo

同时保留 commit 历史但不带 tags。

对每个源仓库（`kosong`、`kaos`）都按下面流程做（在 `Gitrekt-cli` 仓库里操作）：
...
3. 如果 `Gitrekt-cli` 里原来是通过相对路径/本地 editable 依赖来引用 `kosong`/`kaos`，把它们改为正常依赖（`kosong`、`pykaos`），并靠 `tool.uv.sources` 在本地走 workspace。
4. 在 monorepo 根做一次全量自检：
    - `uv sync`（或你们现在用的等价命令）
    - `uv run -m pytest` / `uv run Gitrekt-cli` 的基本命令（按你们项目实际）

目标是：workspace 内能同时开发运行。

## 5. 把 CI/Release workflow 拆成三个

让 tag 触发互斥，且只有 `Gitrekt-cli` 创建 GitHub Release。

核心原则：只有「纯数字 tag」触发 `Gitrekt-cli` release；只有 `kosong-` 触发 `kosong` 发布；只有 `pykaos-` 触发 `pykaos` 发布。这样用户在 `Gitrekt-cli` 仓库的 GitHub Releases 页只会看到 `Gitrekt-cli` 的 release。

### 5.1 Gitrekt-cli release workflow
...
- **构建**：`uv build --package Gitrekt-cli`，并且建议发布路径上用 `--no-sources` 做一次「发布语义」构建，避免 workspace sources 掩盖问题。
...
2. 三个 workflow 在 build 前都调用它：
    - `Gitrekt-cli`：期望版本 = `${GITHUB_REF_NAME}`，pyproject = `./pyproject.toml`
...
1. 在 `Gitrekt-cli` 仓库开迁移分支，先完成 workspace 配置与 subtree 导入，跑通本地开发与测试。
...
4. 最后按原方式发布 `Gitrekt-cli` 的数字 tag（`0.68.1` 之类），验证 GitHub Release 页仍只出现 `Gitrekt-cli`。
