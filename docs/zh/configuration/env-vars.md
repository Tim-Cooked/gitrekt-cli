...
Gitrekt CLI 支持通过环境变量覆盖配置或控制运行行为。本页列出所有支持的环境变量。

关于环境变量如何覆盖配置文件的详细说明，请参阅 [配置覆盖](./overrides.md)。

## Gitrekt 环境变量

以下环境变量在使用 `Gitrekt` 类型的供应商时生效，用于覆盖供应商和模型配置。

| 环境变量 | 说明 |
| --- | --- |
| `GITREKT_BASE_URL` | API 基础 URL |
| `GITREKT_API_KEY` | API 密钥 |
| `GITREKT_MODEL_NAME` | 模型标识符 |
| `GITREKT_MODEL_MAX_CONTEXT_SIZE` | 最大上下文长度（token 数） |
| `GITREKT_MODEL_CAPABILITIES` | 模型能力，逗号分隔（如 `thinking,image_in`） |
| `GITREKT_MODEL_TEMPERATURE` | 生成参数 `temperature` |
| `GITREKT_MODEL_TOP_P` | 生成参数 `top_p` |
| `GITREKT_MODEL_MAX_TOKENS` | 生成参数 `max_tokens` |
...
| 环境变量 | 说明 |
| --- | --- |
| `GITREKT_CLI_NO_AUTO_UPDATE` | 禁用自动更新检查 |
...
::: tip 提示
如果你通过 Nix 或其他包管理器安装 Gitrekt CLI，通常会自动设置此环境变量，因为更新由包管理器处理。
:::
