...
Gitrekt CLI 提供了丰富的交互功能，帮助你高效地与 AI 协作。
...
- **Shell 模式**：直接执行 Shell 命令，无需离开 Gitrekt CLI
...
::: tip 提示
Thinking 模式需要当前模型支持。部分模型（如 `Gitrekt-k2-thinking-turbo`）始终使用 Thinking 模式，无法关闭。
:::
...
如果剪贴板中是图片，Gitrekt CLI 会自动将图片作为附件添加到消息中。发送消息后，AI 可以看到并分析这张图片。
...
斜杠命令是以 `/` 开头的特殊指令，用于执行 Gitrekt CLI 的内置功能，如 `/help`、`/setup`、`/sessions` 等。输入 `/` 后会自动显示可用命令列表。完整的斜杠命令列表请参考 [斜杠命令参考](../reference/slash-commands.md)。

## @ 路径补全

在消息中输入 `@` 后，Gitrekt CLI 会自动补全工作目录中的文件和目录路径。这让你可以方便地引用项目中的文件：
...
当 AI 需要执行可能有影响的操作（如修改文件、运行命令）时，Gitrekt CLI 会请求你的确认。
...
如果你信任 AI 的操作，或者你正在安全的隔离环境中运行 Gitrekt CLI，可以启用「YOLO 模式」来自动批准所有请求：
