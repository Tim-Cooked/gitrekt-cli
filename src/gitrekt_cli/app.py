...
from gitrekt_cli.cli import InputFormat, OutputFormat
from gitrekt_cli.config import Config, LLMModel, LLMProvider, load_config
from gitrekt_cli.flow import PromptFlow
...
from gitrekt_cli.soul.agent import Runtime, load_agent
...
from gitrekt_cli.utils.logging import StreamToLogger, logger
from gitrekt_cli.utils.path import shorten_home
from gitrekt_cli.wire import Wire, WireUISide
from gitrekt_cli.wire.types import ContentPart, WireMessage
...
def enable_logging(debug: bool = False) -> None:
    ...
    logger.enable("gitrekt_cli")
    if debug:
        logger.enable("kosong")
    logger.add(
        get_share_dir() / "logs" / "gitrekt.log",
...
class GitrektCLI:
    ...
    async def create(
        ...
        *,
        ...
        Raises:
            ...
            ConfigError(GitrektCLIException, ValueError): When the configuration is invalid.
            AgentSpecError(
                GitrektCLIException, ValueError
            ): When the agent specification is invalid.
            InvalidToolError(GitrektCLIException, ValueError): When any tool cannot be loaded.
            MCPConfigError(GitrektCLIException, ValueError): When any MCP configuration is invalid.
            MCPRuntimeError(GitrektCLIException, RuntimeError): When any MCP server cannot be
                connected.
        """
        ...
        if not token:
            ...
            try:
                ...
                raise RuntimeError("Login cancelled by user.") from None

            token = await login_flow()
            if not token:
                raise RuntimeError("Authentication required. Please run 'gitrekt login'.")
            save_token(token)
        ...
        soul = GitrektSoul(agent, context=context, flow=flow)
        return GitrektCLI(soul, runtime, env_overrides)
    ...
    @property
    def soul(self) -> GitrektSoul:
        """Get the GitrektSoul instance."""
        return self._soul
    ...
    async def run(
        ...
        """
        Run the Gitrekt CLI instance without any UI and yield Wire messages directly.
        ...
        Yields:
            WireMessage: The Wire messages from the `GitrektSoul`.
    ...
    async def run_shell(self, command: str | None = None) -> bool:
        """Run the Gitrekt CLI instance with shell UI."""
        from gitrekt_cli.ui.shell import Shell, WelcomeInfoItem
        ...
        if base_url := self._env_overrides.get("GITREKT_BASE_URL"):
            welcome_info.append(
                WelcomeInfoItem(
                    name="API URL",
                    value=f"{base_url} (from GITREKT_BASE_URL)",
                    level=WelcomeInfoItem.Level.WARN,
                )
            )
        if self._env_overrides.get("GITREKT_API_KEY"):
            welcome_info.append(
                WelcomeInfoItem(
                    name="API Key",
                    value="****** (from GITREKT_API_KEY)",
                    level=WelcomeInfoItem.Level.WARN,
                )
            )
        ...
        elif "GITREKT_MODEL_NAME" in self._env_overrides:
            welcome_info.append(
                WelcomeInfoItem(
                    name="Model",
                    value=f"{self._soul.model_name} (from GITREKT_MODEL_NAME)",
                    level=WelcomeInfoItem.Level.WARN,
                )
            )
        else:
    ...
    async def run_print(
        ...
        *,
        ...
        """Run the Gitrekt CLI instance with print UI."""
        from gitrekt_cli.ui.print import Print
    ...
    async def run_acp(self) -> None:
        """Run the Gitrekt CLI instance as ACP server."""
        from gitrekt_cli.ui.acp import ACP
    ...
    async def run_wire_stdio(self) -> None:
        """Run the Gitrekt CLI instance as Wire server over stdio."""
        from gitrekt_cli.ui.wire import WireOverStdio
