...
from gitrekt_cli.config import load_config, save_config
from gitrekt_cli.exception import ConfigError
from gitrekt_cli.platforms import get_platform_name_for_provider, refresh_managed_models
...
from gitrekt_cli.utils.changelog import CHANGELOG
from gitrekt_cli.utils.datetime import format_relative_time
from gitrekt_cli.utils.slashcmd import SlashCommand, SlashCommandRegistry
...
def _ensure_gitrekt_soul(app: Shell) -> GitrektSoul | None:
    if not isinstance(app.soul, GitrektSoul):
        console.print("[red]GitrektSoul required[/red]")
        return None
    return app.soul
...
def help(app: Shell, args: str):
    ...
    from gitrekt_cli.utils.rich.columns import BulletColumns
    ...
    renderables.append(
        BulletColumns(
            Text(
                "Sure, Gitrekt CLI is ready to help! "
...
def version(app: Shell, args: str):
    ...
    from gitrekt_cli.constant import VERSION

    console.print(f"gitrekt, version {VERSION}")


@registry.command
async def model(app: Shell, args: str):
    """Switch LLM model or thinking mode"""
    from gitrekt_cli.llm import derive_model_capabilities

    soul = _ensure_gitrekt_soul(app)
    if soul is None:
        return
...
def feedback(app: Shell, args: str):
    """Submit feedback to make Gitrekt CLI better"""
    import webbrowser

    ISSUE_URL = "https://github.com/MoonshotAI/gitrekt-cli/issues"
...
async def mcp(app: Shell, args: str):
    ...
    if not isinstance(toolset, GitrektToolset):
        console.print("[red]GitrektToolset required[/red]")
        return
    ...
    for name, info in servers.items():
        ...
        if info.status == "unauthorized":
            server_text += " [grey50](unauthorized - run: gitrekt mcp auth {name})[/grey50]"
...
async def login(app: Shell, args: str):
    ...
    from gitrekt_cli.auth import fetch_gitrekt_config, login_flow, save_gitrekt_config, save_token
    ...
    if token:
        ...
        if gitrekt_config:
            ...
            console.print("[yellow]Note: Restart Gitrekt CLI to apply new configuration.[/yellow]")
        else:
