...
from gitrekt_cli.ui.shell.console import console
from gitrekt_cli.ui.shell.prompt import CustomPromptSession, PromptMode, toast
from gitrekt_cli.ui.shell.replay import replay_recent_history
from gitrekt_cli.ui.shell.slash import registry as shell_slash_registry
from gitrekt_cli.ui.shell.slash import shell_mode_registry
from gitrekt_cli.ui.shell.update import LATEST_VERSION_FILE, UpdateResult, do_update, semver_tuple
from gitrekt_cli.ui.shell.visualize import visualize
from gitrekt_cli.utils.envvar import get_env_bool
from gitrekt_cli.utils.signals import install_sigint_handler
from gitrekt_cli.utils.slashcmd import SlashCommand, SlashCommandCall, parse_slash_command_call
from gitrekt_cli.utils.term import ensure_new_line, ensure_tty_sane
from gitrekt_cli.wire.types import ContentPart, StatusUpdate
...
class Shell:
    ...
    async def run(self, command: str | None = None) -> bool:
        ...
        if get_env_bool("GITREKT_CLI_NO_AUTO_UPDATE"):
            logger.info("Auto-update disabled by GITREKT_CLI_NO_AUTO_UPDATE environment variable")
        else:
            self._start_background_task(self._auto_update())

        _print_welcome_info(self.soul.name or "Gitrekt CLI", self._welcome_info)

        if isinstance(self.soul, GitrektSoul):
                ...
                while True:
                    ...
                    if user_input.command in ["exit", "quit", "/exit", "/quit", "/logout"]:
                        if user_input.command == "/logout":
                            from gitrekt_cli.auth import clear_auth_files
    ...
    async def _run_slash_command(self, command_call: SlashCommandCall) -> None:
        from gitrekt_cli.cli import Reload
    ...
    async def _run_soul_command(self, user_input: str | list[ContentPart]) -> bool:
        ...
        try:
            await run_soul(
                ...
                cancel_event,
                self.soul.wire_file if isinstance(self.soul, GitrektSoul) else None,
            )
            return True
        except LLMNotSet:
    ...
    async def _auto_update(self) -> None:
        ...
        if result == UpdateResult.UPDATE_AVAILABLE:
            while True:
                toast(
                    "new version found, run `uv tool upgrade gitrekt-cli` to upgrade",
                    topic="update",
                    duration=30.0,
                )
...
def _print_welcome_info(name: str, info_items: list[WelcomeInfoItem]) -> None:
    head = Text.from_markup("Welcome to Gitrekt!")
    ...
    if LATEST_VERSION_FILE.exists():
        from gitrekt_cli.constant import VERSION as current_version
        ...
        if semver_tuple(latest_version) > semver_tuple(current_version):
            rows.append(
                Text.from_markup(
                    ...
                    "Please run `uv tool upgrade gitrekt-cli` to upgrade.[/yellow]"
                )
            )
