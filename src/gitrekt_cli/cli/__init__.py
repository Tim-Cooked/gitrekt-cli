...
cli = typer.Typer(
    ...
    help="Gitrekt, your next CLI agent.",
)
...
def _version_callback(value: bool) -> None:
    if value:
        typer.echo(f"gitrekt, version {VERSION}")
        raise typer.Exit()
...
def gitrekt(
    ...
    config_file: Annotated[
        ...
        typer.Option(
            ...
            dir_okay=False,
            readable=True,
            help="Config TOML/JSON file to load. Default: ~/.gitrekt/config.toml.",
        ),
    ] = None,
    ...
    acp_mode: Annotated[
        ...
        typer.Option(
            "--acp",
            help="(Deprecated, use `gitrekt acp` instead) Run as ACP server.",
        ),
    ] = False,
    wire_mode: Annotated[
        bool,
    ...
    skills_dir: Annotated[
        ...
        typer.Option(
            ...
            dir_okay=True,
            readable=True,
            help="Path to the skills directory. Default: ~/.gitrekt/skills",
        ),
    ] = None,
    # Loop control
    ...
    from gitrekt_cli.agentspec import DEFAULT_AGENT_FILE, OKABE_AGENT_FILE
    from gitrekt_cli.app import GitrektCLI, enable_logging
    from gitrekt_cli.config import Config, load_config_from_string
    from gitrekt_cli.exception import ConfigError
    from gitrekt_cli.metadata import load_metadata, save_metadata
    ...
    if prompt_flow is not None:
        from gitrekt_cli.flow import PromptFlowError
    ...
    async def _run(session_id: str | None) -> bool:
        ...
        instance = await GitrektCLI.create(
            session,
            config=config,
...
def term(
    ...
    """Run Toad TUI backed by Gitrekt CLI ACP server."""
...
@cli.command()
def acp():
    """Run Gitrekt CLI ACP server."""
    from gitrekt_cli.acp import acp_main
...
if __name__ == "__main__":
    if "gitrekt_cli.cli" not in sys.modules:
        sys.modules["gitrekt_cli.cli"] = sys.modules[__name__]
