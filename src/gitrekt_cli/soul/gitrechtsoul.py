...
from gitrekt_cli.flow import FlowEdge, FlowNode, PromptFlow, parse_choice
from gitrekt_cli.llm import ModelCapability
from gitrekt_cli.skill import Skill, read_skill_text
...
from gitrekt_cli.soul.message import check_message, system, tool_result_to_message
from gitrekt_cli.soul.slash import registry as soul_slash_registry
from gitrekt_cli.soul.toolset import GitrektToolset
from gitrekt_cli.tools.dmail import NAME as SendDMail_NAME
from gitrekt_cli.tools.utils import ToolRejectedError
from gitrekt_cli.utils.logging import logger
from gitrekt_cli.utils.slashcmd import SlashCommand, parse_slash_command_call
...
if TYPE_CHECKING:
    ...
    def type_check(soul: GitrektSoul):
        _: Soul = soul
...
class GitrektSoul:
    ...
    def _make_skill_runner(
        self, skill: Skill
    ) -> Callable[[GitrektSoul, str], None | Awaitable[None]]:
        async def _run_skill(soul: GitrektSoul, args: str, *, _skill: Skill = skill) -> None:
    ...
    async def _agent_loop(self) -> TurnOutcome:
        ...
        if isinstance(self._agent.toolset, GitrektToolset):
