...
from gitrekt_cli.exception import InvalidToolError, MCPRuntimeError
from gitrekt_cli.tools import SkipThisTool
from gitrekt_cli.tools.utils import ToolRejectedError
from gitrekt_cli.wire.types import ContentPart, ToolCall, ToolResult, ToolReturnValue
...
if TYPE_CHECKING:
    ...
    def type_check(gitrekt_toolset: GitrektToolset):
        _: Toolset = gitrekt_toolset


class GitrektToolset:
    ...
    def load_tools(self, tool_paths: list[str], dependencies: dict[type[Any], Any]) -> None:
        """
        Load tools from paths like `gitrekt_cli.tools.shell:Shell`.
    ...
    async def load_mcp_tools(
        ...
        from gitrekt_cli.ui.shell.prompt import toast
            ...
            for server_name, server_info in self._mcp_servers.items():
                ...
                if not await _check_oauth_tokens(server_url):
                    logger.warning(
                        ...
                        "Run 'gitrekt mcp auth {server_name}' first.",
                        server_name=server_name,
                    )
