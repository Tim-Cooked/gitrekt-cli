...
from gitrekt_cli.agentspec import load_agent_spec
from gitrekt_cli.config import Config
from gitrekt_cli.exception import MCPConfigError
from gitrekt_cli.llm import LLM
from gitrekt_cli.session import Session
from gitrekt_cli.skill import (
    Skill,
...
class BuiltinSystemPromptArgs:
    ...
    GITREKT_WORK_DIR: KaosPath
    """The absolute path of current working directory."""
    GITREKT_WORK_DIR_LS: str
    """The directory listing of current working directory."""
    GITREKT_AGENTS_MD: str  # TODO: move to first message from system prompt
...
class Runtime:
    ...
    async def create(
        ...
        return Runtime(
            ...
            session=session,
            builtin_args=BuiltinSystemPromptArgs(
                GITREKT_NOW=datetime.now().astimezone().isoformat(),
                GITREKT_WORK_DIR=session.work_dir,
                GITREKT_WORK_DIR_LS=ls_output,
                GITREKT_AGENTS_MD=agents_md or "",
                GITREKT_SKILLS=skills_formatted or "No skills found.",
            ),
            denwa_renji=DenwaRenji(),
...
async def load_agent(
    ...
    *,
    ...
    Raises:
        ...
        AgentSpecError(GitrektCLIException, ValueError): When the agent specification is invalid.
        ...
        MCPRuntimeError(GitrektCLIException, RuntimeError): When any MCP server cannot be connected.
    """
