...
from gitrekt_cli.tools.utils import ToolResultBuilder, load_desc
...
RG_VERSION = "15.0.0"
RG_BASE_URL = "http://cdn.kimi.com/binaries/gitrekt-cli/rg"
...
def _find_existing_rg(bin_name: str) -> Path | None:
    ...
    assert gitrekt_cli.__file__ is not None
    local_dep = Path(gitrekt_cli.__file__).parent / "deps" / "bin" / bin_name
...
async def _download_and_install_rg(bin_name: str) -> Path:
    ...
    async with new_client_session() as session:
        with tempfile.TemporaryDirectory(prefix="gitrekt-rg-") as tmpdir:
