# ACP Integration Notes (Gitrekt-cli)
...
- **Single-session server**: `GitrektCLI.run_acp()` uses `ACP` -> `ACPServerSingleSession`.
  - Code: `src/gitrekt_cli/app.py`, `src/gitrekt_cli/ui/acp/__init__.py`.
  - Used when running CLI with `--acp` UI mode.
- **Multi-session server**: `acp_main()` runs `ACPServer` with `use_unstable_protocol=True`.
  - Code: `src/gitrekt_cli/acp/__init__.py`, `src/gitrekt_cli/acp/server.py`.
  - Exposed via the `Gitrekt acp` command in `src/gitrekt_cli/cli/__init__.py`.
...
- `session/new`
  - Multi-session: creates a persisted `Session`, builds `GitrektCLI`, stores `ACPSession`.
...
- `session/load`
  - Multi-session only: loads by `Session.find`, then builds `GitrektCLI` and `ACPSession`.
...
- `session/set_mode` and `session/set_model` (no multi-mode/model switching in Gitrekt-cli).
