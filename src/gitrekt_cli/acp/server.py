...
from gitrekt_cli.acp.kaos import ACPKaos
from gitrekt_cli.acp.mcp import acp_mcp_servers_to_mcp_config
from gitrekt_cli.acp.session import ACPSession
from gitrekt_cli.acp.tools import replace_tools
from gitrekt_cli.acp.types import ACPContentBlock, MCPServer
from gitrekt_cli.app import GitrektCLI
from gitrekt_cli.config import LLMModel, load_config, save_config
from gitrekt_cli.constant import NAME, VERSION
from gitrekt_cli.llm import create_llm, derive_model_capabilities
from gitrekt_cli.session import Session
from gitrekt_cli.soul.slash import registry as soul_slash_registry
...
class ACPServer:
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        command = sys.argv[0]
        if command.endswith("gitrekt"):
            args = []
        else:
            idx = sys.argv.index("gitrekt")
        ...
        return acp.InitializeResponse(
            ...
            auth_methods=[
                acp.schema.AuthMethod(
                    ...
                    description=(
                        "Run `gitrekt` command in the terminal, "
    ...
    async def new_session(
        ...
        cli_instance = await GitrektCLI.create(
            session,
        ...
        if isinstance(cli_instance.soul.agent.toolset, GitrektToolset):
            replace_tools(
    ...
    async def set_session_model(self, model_id: str, session_id: str, **kwargs: Any) -> None:
        ...
        assert config.is_from_default_location, "`gitrekt acp` must use the default config location"
